...
ll rec(int total_taken_value, int prev_value)
{
    if(total_taken_value > N || prev_value > N) return 0;

    if(total_taken_value == N) return 1;

    if(dp[total_taken_value][prev_value] != -1) return dp[total_taken_value][prev_value];

    ll ret = 0;

    for(int i=prev_value; i<=N; i++)
        ret += rec(total_taken_value+i,i+1);

    return dp[total_taken_value][prev_value] = ret;
}
...
int main()      ///AC
{
    ...
    ll ans = rec(0,1);  /// initially kisu ney nai and, 1 neoar try kortaci ...
    //cerr << "ans: " << ans << endl;
    pf("%lld\n",ans);
    return 0;
}
