...
ll dp[100000][25];
int ara[20];      /// value of every indexed  character in the string
int len;
int base,k;

ll po(int p)
{
    double x = pow((double)base,p);
    ll y = (ll)x;
    return y%k;     /// ekhane mod kora V.V.I.
}

ll rec(int mask,int r,int p)  /// r means reminder
{
    if(mask == (1<<len)-1)
    {
        if(r == 0) return 1;
    ...
    fi(pos,0,len)
    {
        if(!Check(mask,pos))
        {
            int mask1 = Set(mask,pos);
            ll rim = (ara[pos]%k * po(p)%k)%k;
            rim += r;
            ret += rec(mask1,rim%k,p-1);
        }
    }
...
int main()  ///AC
{
    ...
    loop_tc(tc)
    {
        ...
        {
            if(s[i]>='A' and s[i]<='Z') ara[i] = s[i] - 'A' + 10;
            else ara[i] = s[i] - '0';
        }
        //fi(i,0,len) printf("%d ",ara[i]);
        mem(dp,-1);
        ll ans = rec(0,0,len-1);
        pf("Case %d: ",cas);
        pf1ll(ans);
    }
	return 0;
