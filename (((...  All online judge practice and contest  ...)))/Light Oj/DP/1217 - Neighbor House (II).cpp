...
int dp[1005][5],a[1005];
int n;

int rec(int pos,int ck)
{
    if(pos > n) return 0;
    if(dp[pos][ck] != -1) return dp[pos][ck];
    if(pos == n)
    {
        if(ck) dp[pos][ck] = rec(pos+1,ck);
        else dp[pos][ck] = a[pos] + rec(pos+1,ck);
    }
    else
    {
        if(pos == 1) dp[pos][ck] = max(a[1]+rec(pos+2,1),rec(pos+1,ck));
        else dp[pos][ck] = max(a[pos]+rec(pos+2,ck),rec(pos+1,ck));
    }
    return dp[pos][ck];
}
