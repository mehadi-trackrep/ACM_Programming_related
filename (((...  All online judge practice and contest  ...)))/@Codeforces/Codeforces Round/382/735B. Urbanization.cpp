...
int main()
{
    ...
    if(n1 > n2) swap(n1,n2);
    ll s1=0,s2=0;
    for(int i=0; i<n1; i++) s1 += ara[i];
    for(int i=n1; i<n1+n2; i++) s2 += ara[i];
    double ans = (double)s1/(double)n1 + (double)s2/(double)n2;
    printf("%.6lf\n",ans);
    return 0;
}
...
//    deb(s1);
//    deb(s2);
//    double ans = (double)s1/(double)mn + (double)s2/(double)mx;
//    cout << ans << endl;
//	#ifdef LOCAL_DEFINE
