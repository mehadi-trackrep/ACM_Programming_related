...
int main()      ///AC
{
    ...
    int total = 0;
    int mx = 0;
    int ck = 0,cnt = 0,ck1=0;

    len = s.size();

    for(int i=0; i<=len; i++)
    {
        if(i == len) {if(cnt>=mx){mx = cnt;cnt = 0;}break;}
        if(s[i] == '('){ if(cnt > mx) {mx = cnt;cnt = 0;} cnt = 0;ck = 1;continue;}
        if(s[i] == ')') { if(s[i-1] != '_' && s[i-1] != '(') total++; ck = 0;ck1 = 0;continue;}
        if(ck == 1)
        {
            if(s[i] != '_'){ck1 = 1;continue;}
            if(s[i] == '_' && ck1 == 1) {total++;ck1 = 0;}
        }
        else {if(s[i] != '_') {cnt++;continue;} if(s[i] == '_'){if(cnt > mx){mx = cnt;cnt = 0;}cnt = 0;} }
    }
...
36
FE_i_UhQF_oIh(v__qf)WVa_gND

(_WP_Kxm__WEIn_KZLBS)_XDwNnR_c(_Pv_A)LXO__GEd_R_bTP_hAnZ_____sDL
