...
inline int find_direction(info cur)
{
    /// vertical
    if(cur.y == yy and cur.x < xx) return 1;
    if(cur.y == yy and cur.x > xx) return 2;
    /// horizontal
    if(cur.x == xx and cur.y < yy) return 3;
    if(cur.x == xx and cur.y > yy) return 4;
    ///diagonal 1
    if(abs(cur.x-xx) == abs(cur.y-yy) and cur.x < xx and cur.y < yy) return 5;
    if(abs(cur.x-xx) == abs(cur.y-yy) and cur.x > xx and cur.y > yy) return 6;
    ///diagonal 2
    if(abs(cur.x-xx) == abs(cur.y-yy) and cur.x < xx and cur.y > yy) return 7;
    if(abs(cur.x-xx) == abs(cur.y-yy) and cur.x > xx and cur.y < yy) return 8;

    return -1;
}

inline int dist(info cur)
{
    ///return abs(xx-cur.x) + abs(yy-cur.y); eta hobe na , x,y er value negative hoite pare ... :(
    return max(abs(xx-cur.x),abs(yy-cur.y));
}

inline void update_nearest (info& prev,info cur)
{
    if(prev.type == '?' or (dist(prev) > dist(cur))) prev = cur;
}

int main()      ///AC
{
    ///ios::sync_with_stdio(false); eta sob jaray use jorte ney , problem kore :(
    ...
    while(sf3(n,xx,yy) == 3)
    {
        ...
        while(n--){
            ...
            cur.x = x;
            cur.y = y;
            int dir = find_direction(cur);
            if(dir >= 0) update_nearest(state[dir],cur);
        }
        bool ans = false;
        /// check in verticals and horizontals (R,Q)
        fi(i,1,4)
        {
            if(state[i].type == 'R' or state[i].type == 'Q') ans = true;
        }
        fi(i,5,8)
        {
            if(state[i].type == 'B' or state[i].type == 'Q') ans = true;
        }
        puts((ans) ? "YES" : "NO");
    }
