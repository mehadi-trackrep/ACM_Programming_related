...
{
	string str;
	while(cin >> str)       /// worst case aaaaaaa , output: aaaaaaz , minimum ekta substring er char. gula change korte hobe
                            /// jaite output string < input string , but only ei test case a condition ti khate na ...
    {
        int len = str.size();
        int ck = 0;
        fi(i,0,len-1)
        {
            if(str[i] != 'a')   /// first jekha theke 'a' pabo na , sekhan theke porer 'a' paoar ag porjonto string ti hobe the substring
            {
                int j = i;
                while(str[j]!= 'a' && j < len)
                {
