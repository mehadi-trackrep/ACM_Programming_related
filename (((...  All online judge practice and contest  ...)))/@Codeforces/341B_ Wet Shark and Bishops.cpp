...
{
	int n;
	while(cin >> n)     /// for attack: (x1+y1) == (x2+y2) or, (x1-y1) == (x2-y2)
    {
        ll x,y;
        map <ll,ll> mp1,mp2;     /// ekta diagonale jara ace , sobai , sobai ke attack korte parbe ...
        forinZ_i(n)
        {
            cin >> x >> y;
            mp1[x+y]++;
        ...
        for (auto it : mp1) ans += it.second*(it.second-1)/2;
        for (auto it : mp2) ans += it.second*(it.second-1)/2;
