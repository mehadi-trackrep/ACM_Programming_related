...
int longest(int u)
{
    ...
    {
        int v = graph[u][i];
        int longst = longest(v);
        if(longst > maxi) // maximum value ta nibo ...
        {
            maxi = longst;
            //dir[u] = v;
            fn = v;
        }
        else if(longst == maxi) // maximum value ta nibo ...
        {
            if(fn > v) fn = v;
        }
    }
...
int main()
{
    ...
    while(cin >> n)
    {
        ...
        int longest_path = 0,length,finishing_node;

        for(int i=0; i<graph[start].size(); i++)
        {
            int v = graph[start][i];
            length = longest(v);
            cerr << "-->v and length: " << v << " " << length << endl;
            if(length > longest_path)
            {
                longest_path = length;
                finishing_node = fn;
            }
            else if(length == longest_path)
            {
                if(fn < finishing_node) finishing_node = fn;
            }
        }

        printf("Case %d: The longest path from %d has length %d, finishing at %d.\n\n",++cas,start,longest_path,finishing_node);
