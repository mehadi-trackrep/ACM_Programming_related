...
struct data
{
    ...
    bool operator < (const data &d)const
    {
        ...
        else
        {
            ...
            if(p == d.p and r == d.r and w == d.w) return s < d.s;
            else if(p == d.p and r == d.r) return w > d.w;
            else if(p == d.p) return r > d.r;
        }
    }
};
...
int main()  ///AC
{
    ...
    loop_tc(tc)
    {
        ...
        fi(i,0,n)
        {
            ...
            if(r>=1 and r<=99) pp+=1;
            if(r>=100 and r<=199) pp+=2;
            if(r>=200 and r<=299) pp+=3;
            if(r>=300 and r<=399) pp+=4;
            if(r>399) pp += 5;

            if(w>=1 and w<=3) pp+=1;
            if(w>=4 and w<=6) pp+=2;
            if(w>=7 and w<=8) pp+=3;
            if(w>=9 and w<=10) pp+=4;
            if(w>10) pp += 5;
        ...
        sort(ara,ara+n);
        cout << ara[0].s << " " << ara[0].p << " points" << endl;
    }
