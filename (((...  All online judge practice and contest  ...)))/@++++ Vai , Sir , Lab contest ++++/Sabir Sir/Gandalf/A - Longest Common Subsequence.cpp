...
void solution_print(int i,int j)        /// i and j are index
{
    ...
    if(track[i][j] == 'D')
    {
        ...
        printf("%c %d %d\n",str1[i-1],i,j);     /// therefore , string er index 0 theke start so, index i-1 hoibo ...
    }
    else if(track[i][j] == 'L')
    {
...
int main()
{
    ...
    loop_tc(tc)
    {
        ...
        if(L > 1)   /// therefore, has been said that , every two consecutive elements of z are equal to
                    /// some two elements of x: xa, xb, and y: yc, yd where a < b and c < d.
        {
            printf("case %d Y\n%d\n",cas,L);
