...
int main()
{
    ...
    while(cin >> k >> s)        ///AC
    {
        ...
        ara[ind++] = cnt;   /// last a 1 ta zero dhore pattern ... case: 1 , 01010101 ; 0 , 0000

        ll ans = 0;

        fi(i,0,ind-k-1)     /// every one er jonno ...
        {
            if(k == 0) ans += (ara[i]*(ara[i]-1))/2;
            else ans += ara[i]*ara[i+k];
            //cerr << "ara[i] and ara[i+k]: " << ara[i] << ' ' << ara[i+k] << endl;
        }
