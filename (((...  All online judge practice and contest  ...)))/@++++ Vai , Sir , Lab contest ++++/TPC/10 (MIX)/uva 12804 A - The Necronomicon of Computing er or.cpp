...
void rec(int id)
{
    if(id == n+1)
    {
        vis[id] = 2;  /// 2 means finish of working of the id/node
        return ;
    }
    /// if(flag == true || vis[id] == 2) return ; ekhane flag == true dile , ami last instruction a exactly reach korte partaci ki na seta bojha jabe na :(
    if(vis[id] == 2) return ;
    if(vis[id] == 1)
    {
        //deb(0); /// ..........................
    ...
    if(v[id].ff == 'A' || v[id].ff == 'C') rec(id+1);
    if(v[id].ff == 'J' || v[id].ff == 'C') rec(v[id].ss);
...
int main()      ///AC
{
    ...
    loop_tc(tc)
    {
        ...
        mem(vis,0);
        v.clear();
        v.push_back(make_pair('H',-1));  /// for starting of id no. 1
        ...
        fi(i,1,n)
        {
            ...
            int id = 0;
            if(ch != 'A') scanf(" %d",&id);
            v.push_back(make_pair(ch,id));
        }
        rec(1);
        if(vis[n+1] == 0) printf("NEVER\n");
        else if(flag) printf("SOMETIMES\n");
        else printf("ALWAYS\n");
    }
