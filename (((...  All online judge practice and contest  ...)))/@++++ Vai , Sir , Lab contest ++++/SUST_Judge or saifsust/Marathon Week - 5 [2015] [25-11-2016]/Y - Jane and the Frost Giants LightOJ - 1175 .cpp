...
void bfsF(int sx,int sy)
{
    ...
    while(!q.empty())
    {
        ...
        for(int i=0 ; i<4 ; i++)
        {
            ...
            if(vx >=0 && vx <N && vy >=0 && vy <M && !vis[vx][vy] && graph[vx][vy] == '.')
            {
                if(level[vx][vy] <= level[U.first][U.second] + 1) continue;
...
int bfsJ(int sx,int sy)
{
    if(sx == 0 or sx == N-1 or sy == 0 or sy == M-1) return 1;
    int vis[sz][sz] = {{0}};
    queue< pair<int,int> >q ;
    q.push(make_pair(sx,sy));
    level[sx][sy] = 0;
    ...
    while(!q.empty())
    {
        ...
        for(int i=0 ; i<4 ; i++)
        {
            ...
            if(vx >=0 && vx <N && vy >=0 && vy <M && !vis[vx][vy] && graph[vx][vy] == '.')
            {
                if(level[vx][vy] <= level[U.first][U.second] + 1) continue;
                level[vx][vy] = level[U.first][U.second] + 1;
                if(vx == 0 or vx == N-1 or vy == 0 or vy == M-1) return level[vx][vy]+1;
                q.push(make_pair(vx,vy)) ;
            }
        }
    }
...
int main()
{
    ...
    loop_tc(tc) ///AC
    {
        ...
        {
            if(graph[i][j] == '.') level[i][j] = 999999;
        }
        int jx,jy;
        fi(i,0,N)
        fi(j,0,M)
        {
            if(graph[i][j] == 'F') bfsF(i,j);
            else if(graph[i][j] == 'J') {jx=i;jy=j;}
        }
        int res = bfsJ(jx,jy);
        if(res) pf("Case %d: %d\n",cas,res);
