...
void bfs(int SN,int des,int rome)
{
    queue <int> q;
...
int main()  ///AC
{
    ...
    while(tc--)
    {
        ...
        {
            cin >> u >> v;
            if(!mp1[u[0]]) {mp1[u[0]] = x++; mp2[x-1] = u[0];}
            if(!mp1[v[0]]) {mp1[v[0]] = x++; mp2[x-1] = v[0];}
            graph[mp1[u[0]]].pb(mp1[v[0]]);
            graph[mp1[v[0]]].pb(mp1[u[0]]);
            if(u[0] == 'R') rome = mp1[u[0]];
            else if(v[0] == 'R') rome = mp1[v[0]];
        }
        while(q--)
        {
            cin >> u >> v;
            int sn = mp1[u[0]],dn = mp1[v[0]];
            bfs(sn,dn,rome);
            vector<int> vec;
            while(parent[dn] != dn)
            {
