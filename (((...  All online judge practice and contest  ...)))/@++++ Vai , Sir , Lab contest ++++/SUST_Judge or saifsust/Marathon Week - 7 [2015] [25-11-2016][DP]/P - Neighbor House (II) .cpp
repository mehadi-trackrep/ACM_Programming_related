...
int rec(int pos,int limit)
{
    if(pos>limit) return 0;
    if(dp[pos] != -1) return dp[pos];
    int ret;
    ret = max(ara[pos]+rec(pos+2,limit),rec(pos+1,limit));
    return dp[pos] = ret;
}
...
int main()  ///Ac
{
    ...
    {
        sf1(n);
        fi(i,1,n+1) sf1(ara[i]);
        mem(dp,-1);
        int ans = rec(1,n-1);   /// 1 nile n nite parbo na:(
        //deb(ans);
        mem(dp,-1);
        ans = max(ans,rec(2,n)); /// abar, /// 2 nile n nite parbo :)
        pf("Case %d: ",cas);
        pf1(ans);
    }
	return 0;
