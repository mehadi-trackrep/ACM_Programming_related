...
***/
int n;
int value[55],freq[55];
int dp[55][1005];

int rec(int ith,int amount)
{
    if(ith>=n)
    {
    ...
    if(dp[ith][amount] != -1) return dp[ith][amount];   /// ith == position of ith coin
    int ret1=0,ret2=0;
    fi(j,1,freq[ith]+1) {if(amount-value[ith]*j>=0) ret1 += rec(ith+1,amount-value[ith]*j); else break;}
    ret2 += rec(ith+1,amount);
    return dp[ith][amount] = (ret1+ret2)%mod;
}
