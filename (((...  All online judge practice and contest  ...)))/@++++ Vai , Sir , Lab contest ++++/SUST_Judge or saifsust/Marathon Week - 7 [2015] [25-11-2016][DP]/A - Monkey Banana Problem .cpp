...
int rec(int src_posx,int src_posy)      ///AC
{
    if(src_posx >= r || src_posy > c) return 0;
    if(dp[src_posx][src_posy] != -1) return dp[src_posx][src_posy];
    int ret;
    ret = max(ara[src_posx][src_posy]+rec(src_posx+1,src_posy),ara[src_posx][src_posy]+rec(src_posx+1,src_posy+1));
    return dp[src_posx][src_posy] = ret;
}
...
int main()
{
    ...
    loop_tc(tc)
    {
        ...
        {
            fi(j,1,n+1) ara[i][j] = 0;
        }
        fi(i,1,n+1)
        {
            fi(j,1,i+1) sf1(ara[i][j]);
        }
        fi(i,n+1,r)
        {
            fi(j,i-n+1,c+1) sf1(ara[i][j]);
        }
//        fi(i,1,r)
//        {
//            fi(j,1,n+1) printf("%d ",ara[i][j]);
//            cerr << endl;
//        }
        mem(dp,-1);
        int ans = rec(1,1);
