...
int rec(int pos,int colr)
{
    if(pos >= n) return 0;
    if(dp[pos][colr] != -1) return dp[pos][colr];
    int ret;
    if(colr == 0) ret = min(ara[pos][colr]+rec(pos+1,1),ara[pos][colr]+rec(pos+1,2));
    else if(colr == 1) ret = min(ara[pos][colr]+rec(pos+1,0),ara[pos][colr]+rec(pos+1,2));
    else ret = min(ara[pos][colr]+rec(pos+1,0),ara[pos][colr]+rec(pos+1,1));
    return dp[pos][colr] = ret;
}
