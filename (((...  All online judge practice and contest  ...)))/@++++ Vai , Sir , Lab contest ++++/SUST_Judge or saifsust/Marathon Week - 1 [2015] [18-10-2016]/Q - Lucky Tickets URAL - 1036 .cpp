...
Bigint rec(int pos,int val) ///AC
{
    if(val == 0 and pos == n) return Bigint("1");
    if(pos == n) return Bigint("0");
    Bigint &ret = dp[pos][val];
    Bigint a("-1");
    if(ret == a);
    else return ret;
    ret = "0";
    ...
    {
        if(val >= i) ret = ret + rec(pos+1,val-i);
    }
    return ret;
}
...
int main() {
    int s;
    sf2(n,s);
    if(s%2) {pf1(0);return 0;}
    fi(i,0,n+1)
    fi(j,0,s+1) dp[i][j] = "-1";
    Bigint res = rec(0,s/2);
    res = res*res;
    res.print();
    return 0;
}
