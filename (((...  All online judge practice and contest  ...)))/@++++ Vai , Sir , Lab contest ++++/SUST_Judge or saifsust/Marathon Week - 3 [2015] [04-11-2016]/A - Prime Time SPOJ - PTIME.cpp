...
void prime_factorization(int n)
{
    ...
    for(int j=2; j<=n; j++){
        int x=j;
        if(!com[x]) {pcnt[x]++;mxp = max(mxp,x);continue;}
        ...
        if(x>1) {mxp = max(mxp,x);pcnt[x]++;}
        //deb(x);
    }
    //deb(-1);
    int ck=0;
    for(int i=0; prime[i]<=mxp; i++)
    {
        if(!prime[i]) continue;
        if(!ck) {pf("%d^%d",prime[i],pcnt[prime[i]]);ck=1;}
        else pf(" * %d^%d",prime[i],pcnt[prime[i]]);
    }
    cout << endl;
}
