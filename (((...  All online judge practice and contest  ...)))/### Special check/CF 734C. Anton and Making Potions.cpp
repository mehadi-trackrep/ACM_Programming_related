...
//        cin.tie(nullptr);
    ...
    while(sf3ll(n,m,k) == 3)
    {
        ...
        fi(i,0,m-1) sf1ll(a[i]);
        fi(i,0,m-1) sf1ll(b[i]);

        fi(i,0,k-1) sf1ll(c[i]);
        fi(i,0,k-1) sf1ll(d[i]);

        ll ans = n*x;   /// initial ans (state - 01) ...................................................

        /// first a only c,d  and a,b theke niye dekhbo ...

        fi(i,0,k-1)  /// c,d theke minimum ans find (state - 02) ........................................
        {
            if(d[i] <= s) ans = min(ans,x*(n-c[i]));
        }

        fi(i,0,m-1)
        {
            if(b[i] <= s) ans = min(ans,n*a[i]); /// a,b theke minimum ans find (state - 03) .............
            if(b[i] >= s) continue;
            /// a,b & c,d both theke minimum ans find (state - 04) .......................................
            ll val = s - b[i];     /// extra points
            int ind = upper_bound(d,d+k,val) - d;
            deb(ind);
            ind--;
            val = a[i]*(n-c[ind]);
            ans = min(ans,val);
