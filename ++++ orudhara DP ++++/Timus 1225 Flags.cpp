...
int n,dp[3][45+5];     /// 1 == Blue, 2 == RED , 3 == White

int rec(int pre_colour,int pos)
{
    if(pos == n) {
        if(pre_colour == 1) return 0;
        return 1;
    }
    if(dp[pre_colour][pos] != -1) return dp[pre_colour][pos];
    int ret = 0;
    if(pre_colour != 1) {   /// only red niye kaj kortaci ...
        ret += rec(1,pos+1);
        ret += rec(2,pos+1);
    }
    else ret += rec(2,pos+1);
    return dp[pre_colour][pos] = ret;
}
