...
int rec(int l,int r)    /// l = left , r = right
{
    if(l>r) return 0;
    if(dp[l][r] != -1) return dp[l][r];
    if(l == r) {cnt++; dp[l][r]=1; return 1;}
    int temp=0;
    temp = (temp+rec(l,r-1))%mod;
    temp = (temp+rec(l+1,r))%mod;
    temp = (mod+temp-rec(l+1,r-1))%mod;
    //cerr << "-=-=-=> l,r and value,temp: " << l << " " << r << " " << s[l] << " " << s[r] << " "  << temp << endl;
    if(s[l] == s[r]){
        cnt = (cnt+rec(l+1,r-1)+1)%mod;
        temp = (temp+rec(l+1,r-1)+1)%mod;
    }
    return dp[l][r] = temp;
}
