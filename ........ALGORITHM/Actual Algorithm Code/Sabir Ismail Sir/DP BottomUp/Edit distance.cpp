        ...
        for(int j=1; j<=len1; j++)
        {
            ...
            else    /// if not matched ...
            {
                ...
                if( (M[i-1][j]+1) <= (M[i][j-1]+1) )
                {
                    ...
                    lcs[i][j].first_char = str1[j-1];
                    lcs[i][j].second_char = str2[i-1];
...
int main()
{
    ...
    while(M[x][y] != 0)
    {
        ...
        {
            piar.first = lcs[x][y].first_char;
            piar.second = lcs[x][y].second_char;
    ...
    cerr << "somossa character gula: " << endl << endl;
    ...
    cerr << "and EDIT DISTANCE: " << M[len2][len1] << endl;     /// len1 means number of columns and, len2 means number of rows ...
