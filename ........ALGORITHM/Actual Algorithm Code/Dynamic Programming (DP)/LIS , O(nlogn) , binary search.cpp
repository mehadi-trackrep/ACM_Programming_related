...
void path_print(int max_pos,int pos_of_max_pos)
{
    solution.clear();
    solution.pb(ara[pos_of_max_pos]);
    fi(i,pos_of_max_pos-1;i--)
    {
        if
    }
}

int main()      /// O(nlogn) approach   ...
{
    cin >> n;
    fi(i,1,n)
    {
        cin >> num;
        ara[i] = num;
        vector <int> :: iterator it = lower_bound(LIS.begin(),LIS.end(),num);   /// num er soman leftmost same value / tar immidiate boro value er pointer
        if(LIS.end() == it)
        {
            pos[i] = it-LIS.begin();
            LIS.pb(num);
