...
int dp[SIZ][SIZ];   /// number of ways ...

int bottom_up()
{
    /// first kaj, corner sub-problem gula calculate kora ...

    for(int i=sx+1; i<=dx; i++) dp[i][sy] = 1;      /// 1 means 1 vabeyi ashte parbo ...
    for(int j=sy+1; j<=dy; j++) dp[sx][j] = 1;

    for(int i=sx+1; i<=dx; i++)
    {
        for(int j=sy+1; j<=dy; j++)
            dp[i][j] = dp[i][j-1] + dp[i-1][j]; ///  (i,j) point a ashte  (i-1,j) or, (i,j-1) point a age ashte hobe ...so, total ways is the summation of the two points ...
    }
    return dp[dx][dy];
}
