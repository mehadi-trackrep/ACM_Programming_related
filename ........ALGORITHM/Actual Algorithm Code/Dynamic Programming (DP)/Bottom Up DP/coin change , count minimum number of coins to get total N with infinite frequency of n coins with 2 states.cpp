...
void dp()
{
    ...
    /// Let, sob 1 to ith coin theke 0 make korte minimum coins infinity. (V.V.I.)
    ...
    fi(i,1,n_coins)     /// row borabor coin gula ...
    fi(j,0,make_value)  /// column borabor make value gula ...
    {
        if(j >= coins[i])
        {
            if(store[i-1][j] > 1+store[i][j-coins[i]])
            {
                store[i][j] = 1+store[i][j-coins[i]];
                path[j] = i;
            }
...
void PrintCoinCombination (int N)
{
    if(path[N] == -1)
    {
...
int main()
{
    ...
    fi(i,1,n_coins) cin >> coins[i];

	while(cin >> make_value)
    {
        dp();
        printf("%d\n",store[n_coins][make_value]);      /// COMPLEXITY : O(number of coin*make_value)
        PrintCoinCombination(make_value);
    }
