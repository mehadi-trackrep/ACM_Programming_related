...
void dp()
{
    ...
    fi(i,0,n_coins) way[i][0] = 1;      /// sob 1 to ith coin theke 0 make korar way only one (1) . (V.V.I.)

    fi(i,1,n_coins)     /// row borabor coin gula ...
    fi(j,0,make_value)  /// column borabor make value gula ...
    {
        /// 1 to ith coin niye 1 theke make_value porjonto make kora jay kina dekhte ci ...
        if(j >= coins[i]) way[i][j] = way[i-1][j] + way[i][j-coins[i]];
...
int main()
{
    ...
    {
        dp();
        printf("%d\n",way[n_coins][make_value]);      /// COMPLEXITY : O(number of coin*make_value)
    }
