...
int Set(int N, int pos) /// ekhane musk tay N
{
    return N = N | (1 << pos);
}
...
{
    return (N & (1 << pos)) ? 1 : 0;   /// if, (N & (1 << pos)) > 0 then 1 , else 0 ...
}

int rec(int mask)
{
    if(mask == (1<<n)-1) return 0;      /// means musk er sob bits on
    ...
    for(int i=0; i<n; i++)
    {
        if(Check(mask,i) == 0)
        {
            int price = cost[i][i];
            for(int j=0; j<n; j++)
                if(i!=j and Check(mask,j)) price += cost[i][j];    /// ekhon i th job er kaj korbo ... so, etay total price hobe age jeshob job ( 'j' means that's job)  sesh hoye gece tader sapekkhe additional cost ti ...
            int ret = price + rec(Set(mask,i));
