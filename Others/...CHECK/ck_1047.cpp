...
int rec(int i,int j)
{
    if(i>=1 and i <= n and j >=1 and j <= 3)
    {
        if(dp[i][j] != -1) return dp[i][j];
        int ret;
        if(j == 1)
        {
            ret = rec(i+1,2)+ara[i][j];
            ret = min(ret,rec(i+1,3)+ara[i][j]);
        }
        else if(j == 2)
        {
            ret = rec(i+1,1)+ara[i][j];
            ret = min(ret,rec(i+1,3)+ara[i][j]);
        }
        else if(j == 3)
        {
            ret = rec(i+1,1)+ara[i][j];
            ret = min(ret,rec(i+1,2)+ara[i][j]);
        }
        return dp[i][j] = ret;
    }
    else return 0;
...
int main()
{
	...
	loop_tc(tc)
	{
	    ...
	    fi(j,1,3) cin >> ara[i][j];

        memset(dp,-1,sizeof(dp));

        int ans = rec(1,1);     /// duti state dorkar ... position (i,j)
        ans = min(ans,rec(1,2));
        ans = min(ans,rec(1,3));
