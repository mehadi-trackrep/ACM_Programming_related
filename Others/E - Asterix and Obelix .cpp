...
#define pinf	    	1<<28		/// positive infinite
#define ninf	    	(-1)*1<<28	/// negative infinite
...
{
    for(int freq=1; freq<=5; freq++)        /// ichhamoto , dorkar onusare :)
    for(int k=1; k<=n; k++){
        for(int i=1; i<=n; i++){
            for(int j=1; j<=n; j++)
            {
                if((cost[i][j].ff+cost[i][j].ss) > (cost[i][k].ff+cost[k][j].ff+max(cost[i][k].ss,cost[k][j].ss)))
                {
                    //deb(k);
                    cost[i][j].ff = cost[i][k].ff+cost[k][j].ff;
                    cost[i][j].ss = max(cost[i][k].ss,cost[k][j].ss);
                }
            }
        }
    }
...
int main()  ///AC
{
    int n,m,q;
    int tc=1;
    while(sf3(n,m,q) and (n+m+q))
    {
        fi(i,1,n+1)
        fi(j,1,n+1) cost[i][j] = make_pair(pinf,0);

        fi(i,1,n+1) sf1(fst_cost[i]);
        ...
        fi(i,0,m)
        {
            ...
            if(cost[u][v].ff > w)
            {
                cost[u][v].ff = cost[v][u].ff = w;  /// weight
                cost[u][v].ss = cost[v][u].ss = max(fst_cost[u],fst_cost[v]);  /// feast cost
            }
        }
        ...
        fi(i,0,q)
        {
            int u,v;
            sf2(u,v);
            if(cost[u][v].ff == pinf) pf("-1\n");
            else printf("%d\n",cost[u][v].ff+cost[u][v].ss);
        }
    }
	return 0;
}
