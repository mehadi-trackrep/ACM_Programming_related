...
int main()
{
    ...
    loop_tc(tc)
    {
        ...
        fi(i,1,N)
        {
            ...
            if(ck) break;
            //cerr << "track_color: 1 and 2 -> " << track_color[tree_count][1] << ' ' << track_color[tree_count][2] << endl;
        }
        ...
        //cerr << "==>tree_count: " << tree_count << endl;
...
int knapsack(int ind,int n_bull, int n_cow)        /// "bull" re '1' dara and "cow" re '2' dara indicate kore ...
{
    ...
    {
        //cerr << "knapsack check: --->\n";
        if(n_bull >= B and n_cow >= C) return 1;
        else return 0;
    }
...
{
    queue <int> q;
    visited[SN] = 1;    /// visited '1' means bull and '2' means cow
    q.push(SN);
    ...
    while(!q.empty()){
        ...
        for(int i = 0;i < graph[u].size();i++){
            ...
            else  if(visited[u] == visited[v] && visited[u] != 0 && visited[v] != 0) return 1;
        }
    }
