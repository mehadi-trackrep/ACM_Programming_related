...
int main()      ///AC
{
    ...
    {
        memset(dp,0,sizeof(dp));    /// loop dp te bar-bar memset kora lage ...

        dp[0] = 1;  /// base case

        fi(i,0,n-1)     /// row borabor coin gula ...
        fi(j,1,make_value)  /// column borabor make value gula ...
        {
            if(j >= coin[i])    /// ith coin niye koto vabe make kora jay ta dekhte ci ...
            {
                dp[j] += dp[j-coin[i]];
            }
        }
