...
#define fp(i,a,b)              for(int i=a; i<b; i++)			/// Z means zero
#define fep(i,a,b)              for(int i=a; i<=b; i++)			/// O means one
...
int dfs(int sx, int sy)     ///AC
{
    ...
    for(int i=0; i<8; i++)
    {
        int ux = sx + dx[i];
        int uy = sy + dy[i];
        if(ux >= 0 && ux < N && uy >= 0 && uy < M && !visited[ux][uy]&&graph[ux][uy]=='1') dfs(ux,uy);
    }
    return c;
}
int main()
{
    int tc,cnt=0,maxx=0;int i=0;
    scanf("%d\n\n",&tc);
    while(tc--)
    {
        int t=tc+1;
        N=0;i++;
        while(getline(cin,graph[N]) && graph[N] != "") N++;
        M = graph[0].size();
        maxx=0;
        cnt=0;
        fp(i,0,N)
        ...
        ///c=0; //////total kotoguli choto tel ache
        // getchar();
    }
    return 0;
}
