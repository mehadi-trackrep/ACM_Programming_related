...
{
    if (i > e || j < b)
        return n+1;     /// jehetu sob somoy small value er index search korteci .. so, ekhettere infinite value er index return korte hobe :)
    if (b >= i && e <= j)
        return tree[node];
...
{
    if(e < b) return ;
    ll i = query(1,1,n,b,e);
    int area = ara[i]*(e-b+1);
    if(mx < area) mx = area;
    call(b,i-1);
    call(i+1,e);
}

/// process: at first, 1 to n er vitore sob ceye small value er index ber kore area ber kora , okhan theke left a ekta and right a ekta
/// same query chara and every khettere area ber kore maximum ta save kora ; when b > e hobe then stop / return
