...
int dp[1<<16 + 2];      /// n == 16 hole , 2^16 ti state thakte pare ... so, musk er value maximum , 1<<16 er equal hobe :)
int cost[15][16];

int _set(int musk, int pos)
{
    return (musk | (1 << pos));
}
int _preset(int musk, int pos)
{
    return (musk & ~(1 << pos));
}
int _check(int musk, int pos)
{
    return (musk & (1 << pos)) ? 1 : 0;   /// if, (musk & (1 << pos)) > 0 then 1 , else 0 ...
}

int rec(int musk)
{
    if(musk == (1<<n)-1) return 0;
    if(dp[musk] != -1) return dp[musk];
    ...
    for(int i=0; i<n; i++)
    {
        if(_check(musk,i) == 0)
        {
            int price = cost[i][i];
            for(int j=0; j<n; j++)
                if(i!=j and _check(musk,j)) price += cost[i][j];    /// ekhon i th job er kaj korbo ... so, etay total price hobe age jeshob job ( 'j' means that's job)  sesh hoye gece tader sapekkhe additional cost ti ...
            int ret = price + rec(_set(musk,i));
            min_cost = min(min_cost,ret);
        }
    }
    return dp[musk] = min_cost;
}
