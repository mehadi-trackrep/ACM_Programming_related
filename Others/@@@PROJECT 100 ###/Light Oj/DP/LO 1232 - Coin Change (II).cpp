...
#define mxn	    	105    /// node number    		// node number
#define SIZ     	10005
...
void dp()   /// eta te 1 bare sob value paoa jabe na :( V.V.I. case)
{
    memset(way,0,sizeof(way));      ///AC

    way[0] = 1;  /// base case

    fi(i,1,n_coins)     /// row borabor coin gula ...
    fi(j,1,make_value)  /// column borabor make value gula ...
    {
        if(j >= coins[i])    /// ith coin niye 1 theke make_value porjonto make kora jay kina dekhte ci ...
        {
...
int main()
{
    ...
    loop_tc(tc)
    {
        cin >> n_coins >> make_value;

        fi(i,1,n_coins) cin >> coins[i];
        dp();
        printf("Case %d: %d\n",cas,way[make_value]%100000007);      /// memory COMPLEXITY : O(make_value)
    }
