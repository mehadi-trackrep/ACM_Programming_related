...
//    mp2.clear();
    ...
    store_with_topsort.clear();
}

void top_sort(int src);
...
int main()          ///AC
{
    ...
    loop_tc(tc)
    {
        ...
        {
            if(!visited[i])
                top_sort(i);        /// cycle thaka shotteo jor kore top sort kortace ... so, pore abar dfs cara lage ...
        }

        reverse(store_with_topsort.begin(),store_with_topsort.end());

        memset(visited,0,sizeof(visited));
        int cnt = 0;

        for(int i=0; i<store_with_topsort.size(); i++)
        {
            int U = store_with_topsort[i];
            if(!visited[U])
            {
                cnt++;
                top_sort(U);
...
void top_sort(int src)
{
    ...
    {
        int a = graph[src][i];
        if(!visited[a]) top_sort(a);
    }

    store_with_topsort.pb(src);
}
