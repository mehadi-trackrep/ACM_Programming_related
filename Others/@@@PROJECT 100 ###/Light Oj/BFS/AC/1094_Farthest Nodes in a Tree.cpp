...
int n;
int node;
map <pair <int,int> , int > mp;     ///AC

int bfs(int src)       ///for max_weight
{
    int u , i , v ;
    queue<int>q ;
    q.push(src);
    int visited[30005] = {0},weight_level[30005];
    visited[src] = 1;
    int max_weight = 0;
    weight_level[src] = 0;
    ...
    while(!q.empty())
    {
        ...
        {
            v = graph[u][i] ;
            pair <int,int> U = make_pair(u,v);
            pair <int,int> V = make_pair(v,u);
            if(!visited[v])
            {
                visited[v] =  1 ;
                q.push(v) ;
                weight_level[v] = max(mp[U],mp[V]) + weight_level[u];

                if(weight_level[v] >= max_weight)
                {
                    max_weight = weight_level[v];
                    node = v;
                }
            }
        }
