...
void dfs(int sx, int sy)        ///AC
{
    visited[sx][sy] = 1;
    vec.pb(make_pair(sx,sy));
    for(int i=0; i<4; i++)
    {
        int ux = sx + dx[i];
        int uy = sy + dy[i];

        if(ux >= 0 && ux < N && uy >= 0 && uy < M && !visited[ux][uy] && graph[ux][uy] != '#')
        {
           // cerr << sx << ' ' << sy << endl;
            visited[ux][uy] = 1;
            if(graph[ux][uy] == 'C') cnt++;
...
int main()
{
    ...
    loop_tc(tc)
    {
        ...
        {
            cnt = 0;
            if(!visited[i][j] && graph[i][j] != 'C' && graph[i][j] != '#') dfs(i,j);
           // cerr << "cnt: " << cnt << endl;
            for(int k=0; k<vec.size(); k++) save[vec[k].ff][vec[k].ss] = cnt;
           // for(int k=0; k<vec.size(); k++) cerr << vec[i].ff << ' ' << vec[i].ss << endl;
            vec.clear();
        }
        pf("Case %d:\n",cas);
        while(Q--)
