...
int main()  ///AC
{
    ...
    {
        sf1(q);
        int left = 1,right = 1000000000,mid;    /// ekhettere (binary search) right value ta ektu beshi rakhle algo ti thik ans dey ...
        while(left <= right)
        {
            mid = (left+right) >> 1;
            if(count_trailing_zeroes(mid) >= q) right = mid-1;
            else left = mid+1;
        }
        if(count_trailing_zeroes(left) == q) printf("Case %d: %d\n",cas,left);
