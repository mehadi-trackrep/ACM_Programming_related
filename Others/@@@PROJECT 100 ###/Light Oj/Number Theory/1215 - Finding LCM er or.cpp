...
int main()  ///AC
{
    ...
    loop_tc(tc)
    {
        sf3ll(a,b,L);
        ll lcm = LCM(a,b);
        if(L < lcm) {printf("Case %d: impossible\n",cas);continue;}  /// or, L%lcm > 0 holeo impossible :) (V.V.I.)
        ll multi = L/lcm;
        ll c;
        int ck = 0;
        for(int i=1; i<=lcm; i++)
...
=>    L/lcm = c/(1 to lcm)
    where, multi = L/lcm;
    and GCD(lcm,c) er range 1 to c or 1 to lcm , so highest value lcm dhore check kortaci ...
