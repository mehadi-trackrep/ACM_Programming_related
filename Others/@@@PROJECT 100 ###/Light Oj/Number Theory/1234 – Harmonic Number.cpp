...
/// not possible so, size ta reduce korci by 100 -> 100000000/100 = 1000000 korci , jetay H[1] means
/// 1 to 100 er summation , H[2] means 1 to 200 er summation etc ...
...
int main(){ ///AC
    ...
    while( test-- ){
        scanf("%d", &n);
        double ans = H[n/100];
        for(int i = (n/100) * 100 + 1 ; i <= n; i++) ans += 1.0/i;
        printf("Case %d: %.10lf\n", cs++, ans );
    }
    return 0;
}
